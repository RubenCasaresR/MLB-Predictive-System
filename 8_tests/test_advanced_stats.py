...
class TestCalculateWoba:
    ...
    def test_zero_denominator_returns_zero(self):
        woba = calculate_woba(
            ...
            at_bats=0, sacrifice_flys=0,
        )
        assert woba == 0.0

    def test_no_sacrifice_flys(self):
        woba = calculate_woba(
            walks=10, hit_by_pitch=2,
            singles=10, doubles=5, triples=1, home_runs=3,
            at_bats=50,
        )
    ...
    def test_pandas_series(self):
        ...
        woba = calculate_woba(
            df["walks"], df["hit_by_pitch"],
            df["singles"], df["doubles"],
            df["triples"], df["home_runs"],
            df["at_bats"],
        )
    ...
    def test_season_2023_weights(self):
        ...
        woba = calculate_woba(
            walks=10, hit_by_pitch=1,
            singles=10, doubles=5, triples=1, home_runs=2,
            at_bats=50, season=2023,
        )
        woba_2024 = calculate_woba(
            walks=10, hit_by_pitch=1,
            singles=10, doubles=5, triples=1, home_runs=2,
            at_bats=50, season=2024,
        )
...
class TestCalculateWobaWithSplits:
    def test_returns_all_keys(self):
        result = calculate_woba_with_splits(
            stats_vs_lhp={"walks": 5, "hit_by_pitch": 0, "singles": 8, "doubles": 2, "triples": 0, "home_runs": 1, "at_bats": 30},
            stats_vs_rhp={"walks": 10, "hit_by_pitch": 1, "singles": 15, "doubles": 5, "triples": 1, "home_runs": 3, "at_bats": 70},
        )
    ...
    def test_splits_differ(self):
        result = calculate_woba_with_splits(
            stats_vs_lhp={"walks": 1, "hit_by_pitch": 0, "singles": 5, "doubles": 0, "triples": 0, "home_runs": 0, "at_bats": 30},
            stats_vs_rhp={"walks": 10, "hit_by_pitch": 2, "singles": 20, "doubles": 8, "triples": 2, "home_runs": 5, "at_bats": 100},
        )
        # Better vs RHP
    ...
    def test_total_pa_correct(self):
        result = calculate_woba_with_splits(
            stats_vs_lhp={"walks": 5, "hit_by_pitch": 1, "singles": 8, "doubles": 2, "triples": 0, "home_runs": 1, "at_bats": 30},
            stats_vs_rhp={"walks": 10, "hit_by_pitch": 2, "singles": 15, "doubles": 5, "triples": 1, "home_runs": 3, "at_bats": 70},
        )
    ...
    def test_missing_walks_key_raises_error(self):
        with pytest.raises(TypeError):
            calculate_woba_with_splits(
                stats_vs_lhp={"singles": 5, "doubles": 1, "triples": 0, "home_runs": 0, "at_bats": 25},
                stats_vs_rhp={"singles": 10, "doubles": 3, "triples": 0, "home_runs": 1, "at_bats": 50},
            )
...
class TestCalculateFip:
    ...
    def test_zero_innings(self):
        fip = calculate_fip(
            strikeouts=0, walks=0, hit_by_pitch=0,
            home_runs=0, innings_pitched=0.0,
        )
        assert fip == 0.0
    ...
    def test_custom_factor(self):
        ...
        fip = calculate_fip(
            strikeouts=100, walks=30, hit_by_pitch=5,
            home_runs=10, innings_pitched=100.0,
    ...
    def test_pandas_series(self):
        ...
        fip = calculate_fip(
            df["strikeouts"], df["walks"], df["hit_by_pitch"],
            df["home_runs"], df["innings_pitched"],
        )
    ...
    def test_season_2022_constant(self):
        fip = calculate_fip(
            strikeouts=0, walks=0, hit_by_pitch=0,
            home_runs=0, innings_pitched=1.0, season=2022,
        )
        # should use 3.15
    ...
    def test_unknown_season_default(self):
        fip = calculate_fip(
            strikeouts=0, walks=0, hit_by_pitch=0,
            home_runs=0, innings_pitched=1.0, season=2025,
        )
...
class TestCalculateXera:
    ...
    def test_with_quality_of_contact(self):
        xera = calculate_xera(
            fip=3.50, quality_of_contact=0.320,
        )
    ...
    def test_all_adjustments(self):
        xera = calculate_xera(
            fip=4.00, quality_of_contact=0.350,
            defense_rating=0.2, park_factor=1.05,
        )
...
class TestCalculateSiera:
    ...
    def test_strikeout_dominant(self):
        siera = calculate_siera(
            strikeouts=300, walks=30,
            ground_balls=150, fly_balls=100,
            innings_pitched=180.0,
        )
    ...
    def test_zero_ground_balls(self):
        siera = calculate_siera(
            strikeouts=100, walks=50,
            ground_balls=0, fly_balls=100,
...
class TestComputeRollingStats:
    ...
    def test_multiple_players(self):
        df = pd.DataFrame({
            "player_id": [1, 1, 2, 2],
            "game_date": pd.to_datetime(["2026-05-01", "2026-05-02", "2026-05-01", "2026-05-02"]),
