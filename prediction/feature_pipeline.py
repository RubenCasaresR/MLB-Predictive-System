...
class FeaturePipeline:
        ...
        with self.engine.begin() as conn:
            ...
            conn.execute(
                ...
                SELECT
                    ...
                    ROUND(
                        SUM(CASE WHEN ab.events IN ('Strikeout','Strikeout Double Play') THEN 1 ELSE 0 END) * 1.0
                        / NULLIF(SUM(CASE WHEN ab.events IS NOT NULL THEN 1 ELSE 0 END), 0) * 27.0
                    , 2) AS k_per_9,
                    ROUND(
                        SUM(CASE WHEN ab.events IN ('Walk','Intent Walk','Hit By Pitch') THEN 1 ELSE 0 END) * 1.0
                        / NULLIF(SUM(CASE WHEN ab.events IS NOT NULL THEN 1 ELSE 0 END), 0) * 27.0
                    , 2) AS bb_per_9,
                    ROUND(
                        SUM(CASE WHEN ab.events = 'Home Run' THEN 1 ELSE 0 END) * 1.0
                        / NULLIF(SUM(CASE WHEN ab.events IS NOT NULL THEN 1 ELSE 0 END), 0) * 27.0
                    , 2) AS hr_per_9,
                    ROUND(CAST(AVG(p.release_speed) AS NUMERIC), 2) AS avg_velo,
                    ROUND(SUM(CASE WHEN p.whiff = TRUE THEN 1 ELSE 0 END) * 100.0
                        / NULLIF(SUM(CASE WHEN p.swing = TRUE THEN 1 ELSE 0 END), 0), 2) AS whiff_pct
                FROM games g
                JOIN at_bats ab ON ab.game_id = g.game_id
            ...
            conn.execute(
                ...
                SELECT
                    ...
                    ROUND(CAST(AVG(CASE
                        ...
                        WHEN ab.events = 'Home Run' THEN 2.00
                        ELSE 0 END) AS NUMERIC), 4) AS woba,
                    ROUND(SUM(CASE WHEN ab.events IN ('Strikeout','Strikeout Double Play') THEN 1 ELSE 0 END) * 1.0
                        / NULLIF(COUNT(*), 0) * 100, 1) AS k_pct,
                    ROUND(SUM(CASE WHEN ab.events IN ('Walk','Intent Walk','Hit By Pitch') THEN 1 ELSE 0 END) * 1.0
                        / NULLIF(COUNT(*), 0) * 100, 1) AS bb_pct,
                    ROUND(COALESCE(
                        SUM(CASE WHEN ab.events = 'Home Run' THEN 1 ELSE 0 END) * 1.0
                        / NULLIF(SUM(CASE WHEN ab.events IS NOT NULL THEN 1 ELSE 0 END), 0) * 9.0, 0
                    ), 2) AS hr_per_9,
                    ROUND(COALESCE(
                        SUM(CASE WHEN ab.launch_angle IS NOT NULL AND ab.launch_angle < 10 THEN 1 ELSE 0 END) * 1.0
                        / NULLIF(SUM(CASE WHEN ab.launch_angle IS NOT NULL THEN 1 ELSE 0 END), 0) * 100, 0
                    ), 1) AS gb_pct,
                    ROUND(COALESCE(
                        SUM(CASE WHEN ab.launch_angle IS NOT NULL AND ab.launch_angle > 25 THEN 1 ELSE 0 END) * 1.0
                        / NULLIF(SUM(CASE WHEN ab.launch_angle IS NOT NULL THEN 1 ELSE 0 END), 0) * 100, 0
                    ), 1) AS fb_pct
                FROM games g
                JOIN at_bats ab ON ab.game_id = g.game_id
    ...
    def detect_sharp_money(self, target_date: date):
            ...
            conn.execute(
                ...
                WITH lagged AS (
                    ...
                    WHERE recorded_at::date = :gd
                      AND home_ticket_pct IS NOT NULL
                      AND home_money_pct IS NOT NULL
                )
                UPDATE market_lines
        ...
        logger.info("Sharp money flags updated (if data available)")
